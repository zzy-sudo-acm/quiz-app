...
作用：适用于当你有题库，并且会考原题的应试考试，直接在 word 文档上看太费眼，这种简易刷题可以快速记忆留下印象。
...
- `index.html`：页面入口
- `style.css`：移动端优先样式
- `app.js`：刷题逻辑、顺序/随机模式、`localStorage` 进度保存、service worker 注册
...
- 顺序刷题和随机刷题的当前位置、作答状态都会保存在 `localStorage`。
- `重新开始` 会重置当前模式进度。
